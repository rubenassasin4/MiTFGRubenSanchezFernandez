...
En la función ``generarResumen()'', se crea una tabla detallada que muestra todos los productos configurados por el usuario, junto con sus precios,
...
la ficha de producto, lo que requiere que el usuario indique la cantidad en el personalizador y luego pulse el botón para abrir el modal 
y configurar el número deseado de productos personalizados), el precio por unidad y el subtotal. Si el producto tiene un canon digital, 
...
En cuanto a la visualización de este resumen, se muestra el total del pedido y un botón para añadir al carrito toda la personalización 
...
productos al carrito. En esta función, se crea un array llamado "items" en el que se van añadiendo todos estos productos, junto con su cantidad 
...
almacenada en "infoProduct". Se verifica si hay un canon digital y se añade o no este producto con su cantidad correspondiente. Luego, se recorren las áreas y se añaden todos los productos asociados con las cantidades adecuadas (trabajos, cargo extra por colores, cargo de doble pasada y cliché). Finalmente, se añade al array el producto principal con su cantidad y propiedades. Se utiliza la API de Ajax para especificar algunos atributos y datos extra en cada producto añadido al carrito. Se asigna un id único a todos los productos pertenecientes al pedido de impresión, que servirá para asociar posteriormente todos estos productos en un solo paquete. Además, se añaden otros atributos, como el área a la que pertenece cada elemento, los números de colores en el cliché, las dimensiones en el trabajo, las observaciones y la imagen seleccionada.

Por último, se realiza una solicitud POST empleando fetch a la API de AJAX de Shopify, añadiendo estos items en el cuerpo 
del mensaje formateado correctamente, y se culmina con la adición de todos estos productos al carrito.
