...
necesidades de los clientes. El diseño se centró en historias de usuario, que sirvieron como guía para comprender las necesidades de los usuarios y 
traducirlas en funcionalidades concretas.

La implementación de la aplicación se dividió en cuatro partes: creación de la aplicación y extensión del tema de la tienda, desarrollo del \textit{backend}, 
elaboración del \textit{frontend} de la parte de administración, y detalles de la implementación de paquetes personalizados (\textit{customize bundles}). Se llevaron a cabo pruebas continuas 
para verificar el funcionamiento de las funcionalidades implementadas, con pruebas manuales en el \textit{frontend} y \textit{backend}.
...
\begin{itemize}
    ...
    la implementación tanto del \textit{frontend} como del \textit{backend}. También se cubren las pruebas realizadas y el proceso de despliegue de la aplicación.
...
Este backlog es dinámico y evoluciona a lo largo del proyecto, y sus elementos se priorizan en función de su valor para el producto (ver \ref{fig:4}) y se ajustan continuamente en función de los cambios en los requisitos o las necesidades del proyecto.

En el ámbito de una empresa, el jefe de proyecto es quien asigna las prioridades a las tareas pertenecientes a cada historia de usuario, y estas prioridades marcan
...
Además esta herramienta cuenta también con una funcionalidad que permite marcar las tareas con diferentes estados como ``Nueva'', ``Activa'', ``Lista para pruebas'', ``Testing'', ``Resuelta'' y ``Cerrada'' (ver \ref{fig:4}). Esto permite un seguimiento claro y conciso del flujo de trabajo de cada tarea
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Al añadir productos al carrito, se deben incluir:
          ...
          \end{itemize}
    \item Se debe investigar si el uso de \textit{customized bundles} es adecuado para agrupar y vincular los productos relacionados en el carrito.
...
quiero que los paquetes de productos de impresión (\textit{customize bundles}), se visualicen completamente de forma agrupada en el carrito de la tienda,
...
\begin{enumerate}[label=\arabic*.]
    ...
    \item Se deben utilizar \textit{customize bundles} para asociar cada línea de producto con su línea de canon digital correspondiente en el carrito.
\end{enumerate}
...
extensión permite la integración de la aplicación (\textit{frontend}) con el tema de la tienda de Shopify.
...
se generará la estructura inicial de la extensión del tema, proporcionando una base sobre la que poder trabajar y desarrollar la parte del \textit{frontend} de la aplicación que se integrará con
...
\clearpage
\subsubsection{Implementación del \textit{frontend} de la parte de administración}
...
En esta sección se mostrará la investigación e implementación de los \textit{customize bundles} para agrupar y vincular los
...
``customize bundles''. Estas funciones se basan en destinos de extensión que inyectan código en la lógica de \textit{backend} de Shopify. La entrada de estas funciónes es un objecto JSON resultado de una
...
Al aplicar los \textit{customize bundles}, se observa que los paquetes de productos de personalización, no muestran los detalles de los artículos que contienen. Estos 
...
\begin{itemize}
    ...
    \item \textbf{Tarea 2:} Crear Producto Especial para Añadir el Importe al Carrito:
          ...
          \end{itemize}
    \item \textbf{Tarea 3:} Añadir Líneas de Canon al Carrito y Asociarlas a sus Productos Correspondientes con \textit{customize bundles}:
          \begin{itemize}[label=--]
              ...
              Para los productos normales, se debe investigar si modificando la funcionalidad de los cart-tranform existente (\textit{customize bundles}), se pueden extender los productos ``normales'' que cuenten con canon digital y que en el carrito, se cree automáticamente un paquete con cada producto normal y su canon asociado, con la cantidad adecuada.
          \end{itemize}
\end{itemize}
