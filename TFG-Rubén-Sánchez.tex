...
proyecto básico, se procedión a analizar, desarrollar e implementar las tareas adicionales. Este enfoque iterativo e incremental permitió una implementación
...
En este apartado se muestra la implementación en sus dos partes: la de las tareas básicas y la de las tareas adicionales. Estas tareas proporcionan una hoja
de ruta clara y organizada para la implementación del proyecto. Esta estructuración facilitó el proceso de implementación al proporcionar una orientación clara 
sobre los pasos a seguir y los objetivos a alcanzar en cada etapa del desarrollo. 
...
Los productos personalizables de las tiendas, deberán contar con sus correspondientes areas de impresión, estas representan
las zonas del producto donde se puede realizar una personalización y cuentan con información relevante sobre dicha zona.
Para representar estas areas se debe crear un metaobjeto de ``Área de impresión''. En la Figura~\ref{fig:metaobjeto} se puede ver la estructura de este 
objeto. Este cuenta con un nombre del área, la imagen que respresnta la zona del producto a personalizar, el alto y ancho máximo que puede tener la impresión y 
...
no se puedan comprar por separado, no permitirá rastrear su cantidad, por lo que no tendrán stock y tampoco requeriran de envio.
...
crear la definición del metacampo Clicé del trabajo, para el tipo de elemento ``Producto''. Esta definición será del tipo ``referencia de produto''. De esta forma al crear los productos trabajos correspondientes se le podrá asociar su producto cliché vinculado.
...
Una vez configurados los elementos necesarios en la tienda, se procede a crear la aplicación. Para ello se va a emplear la Shopify CLI \cite{shopify-cli}, esta herramienta
...
Para generar la aplicación se ejecuta el siguiente comando desde el directorio en el que se desee: 
\begin{lstlisting}
...
Para generar la extensión se volverá a emplear la Shopify CLI y se introducirá el siguiente comando:

\begin{lstlisting}
    shopify app generate extension
\end{lstlisting}

Al introducirlo se selecciona el tipo de extensión que se desea implementar y se le proporciona un nombre.
